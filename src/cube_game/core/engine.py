...
class Engine:
    ...
    def _cube_wrap_around(self):
        if self.GameContext.player.pos.x < 0:
            self.GameContext.player.pos.x = self.GameContext.player.current_room.pos[Corner.TOP_RIGHT].x
        if self.GameContext.player.pos.x > 24:
            self.GameContext.player.pos.x = self.GameContext.player.current_room.pos[Corner.BOTTOM_LEFT].x
        if self.GameContext.player.pos.y < 0:
            self.GameContext.player.pos.y = self.GameContext.player.current_room.pos[Corner.TOP_RIGHT].y
        if self.GameContext.player.pos.y > 18:
            self.GameContext.player.pos.y = self.GameContext.player.current_room.pos[Corner.BOTTOM_LEFT].y
    ...
    def _change_room(self, direction):
        ...
        next_room_color = self.GameContext.player.current_room.doors[direction].leads_to
        # Use RoomColor.ENUM as key to return Room object from map_dict{}
        next_room = self.GameContext.world.map_dict.get(next_room_color)
        # Update direction player is looking
        self.GameContext.player.direction = direction
        # Update Room where player is located now 
        self.GameContext.player.current_room = next_room
    ...
    def _door_infront(self, direction):
        ...
        infront = self.GameContext.player.pos + (dx, dy)
    ...
    def _get_door(self, direction):
        try:
            door = self.GameContext.player.current_room.doors[direction]
        except KeyError:
            return None
        return door
    ...
    def move_straight(self, directional_command):
        ...
        player_axis_val = axis_func(self.GameContext.player.pos + (dx, dy))
        # axis function for room position at corner
        room_axis_val   = axis_func(self.GameContext.player.current_room.pos[corner])
        ...
        if op(player_axis_val, room_axis_val):
            self.GameContext.player.pos.move(dx,dy)
            self.GameContext.player.direction = direction
        ...
        elif door is not None and self.GameContext.player.pos == door.pos:
    ...
    def command_controller(self, command):
        ...
        if handlerKey == CommandHandler.OPTION_COMMAND:
            ...
            mSkey = self.GameContext.stateController.menuState
    ...
    def update(self):
        ...
        self.GameContext.stateController.systemState = SystemState.OK
        ...
        self.command = self.GameContext.inputController.process_input(sc.gameState, sc.menuState)
